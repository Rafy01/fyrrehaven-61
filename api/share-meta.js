...
const OG_IMAGE = "https://media.fyrrehaven-61.dk/wp-content/uploads/2025/09/ogimage2.jpg"; // JPG!
...
function parsePath(pathname) {
    ...
    const lang = (parts[0] === "en" ? "en" : "da");
    const slug = decodeURIComponent(parts[1] || "");
    const key = (REVERSE[lang][slug] ?? "home");
    return { lang, key };
}
const META = {
    home: {
...
function isBot(ua) {
    return /(bot|facebookexternalhit|twitterbot|linkedinbot|whatsapp|slackbot|discord|telegram|pinterest|embedly|quora link preview|vkShare|google-InspectionTool|googleweblight|bingbot)/i.test(ua || "");
}
export default async function handler(req, res) {
    ...
    const canonical = `${BASE_URL}/${lang}${SLUGS[key][lang] ? `/${SLUGS[key][lang]}` : ""}`;
    const hrefDa = `${BASE_URL}/da${SLUGS[key]["da"] ? `/${SLUGS[key]["da"]}` : ""}`;
    const hrefEn = `${BASE_URL}/en${SLUGS[key]["en"] ? `/${SLUGS[key]["en"]}` : ""}`;
