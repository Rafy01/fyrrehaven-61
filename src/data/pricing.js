 ...
 * Defaults
 * ========= */
export const DEFAULT_WEEKEND = ["fri", "sat"];
 ...
 */
export function getMinNightsForStart(date, plan = PRICES) {
    ...
    const wp = yr.weeks?.[w];
    if (wp?.minNights != null)
        return wp.minNights;
    if (yr.default?.minNights != null)
        return yr.default.minNights;
    return GLOBAL_FALLBACK;
}
/* =========
 * Hjælpere til at udfylde days
 * ========= */
function addRange(store, fromYMD, toYMD, price) {
...
function addOne(store, ymd, price) {
    store[ymd] = price;
}
/** Helper til at udfylde dags-specifikke min.-nætter */
function addRangeMinNights(store, fromYMD, toYMD, minNights) {
    const start = new Date(fromYMD + "T00:00:00");
    const end = new Date(toYMD + "T00:00:00");
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        const ymd = d.toISOString().slice(0, 10);
...
const daysMinNights2025 = {};
const daysMinNights2026 = {};
const daysMinNights2027 = {};
/** ——— 2025 ——— **/
// SEPTEMBER 2025
...
export function getPriceForDate(date, plan = PRICES) {
    const y = date.getFullYear();
    const yr = plan.years[y];
    if (!yr)
        return null;
